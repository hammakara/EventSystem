<x-layouts.dashboard title="Edit venue">
    <div class="p-6 max-w-2xl">
        <form method="POST" action="{{ route('venues.update',$venue) }}" enctype="multipart/form-data" class="space-y-4" x-data="{ preview:null }">
            @csrf @method('PUT')
            <div>
            ...
            <div>
                ...
                <input name="address" value="{{ old('address',$venue->address) }}" class="form-control" required>
            </div>
            <div>
                <label class="form-label">Contact</label>
                <input name="contact" value="{{ old('contact',$venue->contact) }}" class="form-control">
            </div>
            <div>
                <label class="form-label">Owner</label>
                <input name="owner" value="{{ old('owner',$venue->owner) }}" class="form-control">
            </div>
            <div>
                ...
                <div class="mt-1 grid grid-cols-1 gap-3 sm:grid-cols-2 items-start">
                    <div class="aspect-video rounded-xl bg-neutral-100 dark:bg-neutral-800 overflow-hidden flex items-center justify-center">
                        <img x-show="preview || '{{ $venue->image }}'" :src="preview ?? '{{ $venue->image ? (Str::startsWith($venue->image, 'http') ? $venue->image : asset('storage/'.$venue->image)) : '' }}'" class="w-full h-full object-cover"/>
                    </div>
            ...
            <div class="flex gap-2">
                <a href="{{ route('venues.index') }}" class="px-3 py-2 rounded-xl border">Cancel</a>
