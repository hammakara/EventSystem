<x-layouts.dashboard title="Edit vendor">
    <div class="p-6 max-w-2xl">
        <form method="POST" action="{{ route('vendors.update',$vendor) }}" enctype="multipart/form-data" class="space-y-4" x-data="{ preview:null }">
            @csrf @method('PUT')
            <div>
            ...
            <div>
                ...
                <input name="contact" value="{{ old('contact',$vendor->contact) }}" class="form-control">
            </div>
            <div>
                <label class="form-label">Fee</label>
                <input name="fee" type="number" step="0.01" value="{{ old('fee',$vendor->fee) }}" class="form-control">
            </div>
            <div>
                <label class="form-label">Details</label>
                <textarea name="details" class="form-textarea">{{ old('details',$vendor->details) }}</textarea>
            </div>
            <div>
                ...
                <div class="mt-1 grid grid-cols-1 gap-3 sm:grid-cols-2 items-start">
                    <div class="aspect-video rounded-xl bg-neutral-100 dark:bg-neutral-800 overflow-hidden flex items-center justify-center">
                        <img x-show="preview || '{{ $vendor->image }}'" :src="preview ?? '{{ $vendor->image ? (Str::startsWith($vendor->image, 'http') ? $vendor->image : asset('storage/'.$vendor->image)) : '' }}'" class="w-full h-full object-cover"/>
                    </div>
