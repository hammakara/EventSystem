<x-layouts.dashboard title="Edit organizer">
    <div class="p-6 max-w-2xl">
        <form method="POST" action="{{ route('organizers.update',$organizer) }}" enctype="multipart/form-data" class="space-y-4" x-data="{ preview:null }">
            @csrf @method('PUT')
            <div>
                <label class="form-label">Name</label>
                <input name="name" value="{{ old('name',$organizer->name) }}" class="form-control" required>
            </div>
            <div>
                <label class="form-label">Email</label>
                <input name="email" type="email" value="{{ old('email',$organizer->email) }}" class="form-control" required>
            </div>
            <div>
                <label class="form-label">Address</label>
                <input name="address" value="{{ old('address',$organizer->address) }}" class="form-control">
            </div>
            <div>
                <label class="form-label">Role</label>
                <input name="role" value="{{ old('role',$organizer->role) }}" class="form-control">
            </div>
            <div>
                ...
                <div class="mt-1 grid grid-cols-1 gap-3 sm:grid-cols-2 items-start">
                    <div class="aspect-square rounded-xl bg-neutral-100 dark:bg-neutral-800 overflow-hidden flex items-center justify-center">
                        <img x-show="preview || '{{ $organizer->image }}'" :src="preview ?? '{{ $organizer->image ? (Str::startsWith($organizer->image, 'http') ? $organizer->image : asset('storage/'.$organizer->image)) : '' }}'" class="w-full h-full object-cover"/>
                    </div>
